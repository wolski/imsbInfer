...
We analyse the distribution of correlations (spearman) among features over multiple samples, and how this correlation is influenced by the samplewise normalization of the data.

We have 5 different cases:
\begin{itemize}
\item random - Features/Peakgroups  can be randomly drawn from the dataset.
\item unique - Features/Peakgroups can be randomly drawn from among those which uniquely represent proteins.
...
rm(list=ls())
library(imsbInfer)
#PanHuman = ("data/E1405020812-PanHuman-canonical/feature_alignment_requant.tsv")
