...
\begin{itemize}
\item random - pairs of transitions are randomly drawn from the dataset.
\item different protein - pairs of transitions are drawn in such way that they belong to different peptides.
\item same protein - pairs of transitions are randomly drawn among peptides of the same protein.
\item same protein TOP - pairs of transitions are randomly drawn among peptides of the same protein and having good intensities.
\item same peptide - pairs of transitions are of the same peptide but with different charge.
\end{itemize}
...
<<fig=TRUE>>=
par(mar=c(10,5,1,1))
barplot(apply(specLib$score,2,function(x){sum(x==2)/length(x)*100}),ylab="% of requant values",xlab="run",las=2)
dim(specLib$score)
@
