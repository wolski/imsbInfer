...
<<fig=TRUE>>=
barplot(apply(specLib$score,2,function(x){sum(x==2)/length(x)*199}),ylab="% of requant values",xlab="run")
@
...
<<fig=TRUE>>=
boxplot((specLib$score),ylab="mscore",log="y",xlab="run",pars=list(pch="."))
@
