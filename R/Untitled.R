...
colnames(dat)
agg1 <- dat[J(c("TripleR", "RSA","httpRequest","protViz","quantable")), length(unique(ip_id)), by=c("weekday", "package")]
agg1[1:28,]
ggplot(agg1, aes(x=weekday, y=V1, color=package, group=package)) + geom_line() + ylab("Downloads") + theme_bw() + theme(axis.text.x  = element_text(angle=90, size=8, vjust=0.5))


agg1 <- dat[J(c("psych", "TripleR", "RSA", "quantable")), length(unique(ip_id)), by=c("week", "package")]
